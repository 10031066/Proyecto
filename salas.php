<?php
 include 'top.php'
?>

<div class="row">
	<div class="col-md-2">
		<ul class="breadcrumb">
          <li><a href="index.php">Home</a><span class="divider">|</span></li>
          <li><a href="salas.php">Salas</a> <span class="divider">|</span></li>
        </ul>
	</div>
</div>

<div class="row">

	<div class="col-md-4">
            <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala1.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 4,799<br>
			           3 pagos desde <br>
			           $ 1,973.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>

    </div>

    <div class="col-md-4">
          <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala2.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 8,799<br>
			           6 pagos desde <br>
			           $ 1,973.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>

    </div>

    <div class="col-md-4">
         <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala3.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 6,799<br>
			           4 pagos desde <br>
			           $ 1,699.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>
    </div>
          
</div>

<div class="row">

	<div class="col-md-4">
            <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala4.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 4,799<br>
			           3 pagos desde <br>
			           $ 1,973.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>

    </div>

    <div class="col-md-4">
          <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala5.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 8,799<br>
			           6 pagos desde <br>
			           $ 1,973.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>

    </div>

    <div class="col-md-4">
         <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala6.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 6,799<br>
			           4 pagos desde <br>
			           $ 1,699.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>
    </div>
          
</div>

<div class="row">

	<div class="col-md-4">
            <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala7.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 4,799<br>
			           3 pagos desde <br>
			           $ 1,973.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>

    </div>

    <div class="col-md-4">
          <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala8.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 8,799<br>
			           6 pagos desde <br>
			           $ 1,973.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>

    </div>

    <div class="col-md-4">
         <div class="panel panel-default">
              <div class="panel-body">

                <div class="col-md-12">
                   <img src="img/sala9.jpg" width="250" onmouseover="javascript:this.height=500;this.width=500"  
                   onmouseout="javascript:this.width=250;this.height=250"/>
              	</div>

              	<div class="col-md-12">
                   <h4><font color="#DF3A01">Sala Esquinero Dimer</font></h4>
      				<HR >

      				<P style="text-align: center;">
			          <span style="color:#424242">$ 6,799<br>
			           4 pagos desde <br>
			           $ 1,699.83 
			         </P>

			         <input class="btn btn-primary" type="submit" value="Agregar al carrito" >

              	</div>


              </div>
            </div>

            <HR>
    </div>
          
</div>



<?php
 include 'bottom.html'
?>
